...
import { UserPlus, Zap, CheckCircle, AlertOctagon, Clock } from 'lucide-react';

const Reception = () => {
  const { addPatient, departments } = useStore();
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: '1.5rem', alignItems: 'start' }}>
        ...
        <div className="card" style={{ padding: '2rem' }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button type="submit" className="btn btn-primary" style={{ width: '100%', marginTop: '1.5rem', padding: '0.8rem', fontSize: '0.9rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <div className="card" style={{ background: 'linear-gradient(135deg, rgba(59,130,246,0.12) 0%, rgba(168,85,247,0.08) 100%)', border: '1px solid var(--border-default)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.875rem' }}>
              ...
              <h3 style={{ color: 'var(--primary)' }}>AI Triage Engine</h3>
            </div>
            <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
              The AI engine analyzes symptoms, patient age, visit type, and wait time to assign a dynamic <strong style={{ color: 'var(--text-primary)' }}>Priority Score</strong>. The queue auto-reorders every 15 seconds.
            </p>
            <div style={{ marginTop: '1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {[
                { label: 'Critical', color: 'var(--critical)',  bg: 'var(--critical-bg)',  desc: 'Emergency override — immediate' },
                { label: 'High',     color: 'var(--high)',      bg: 'var(--high-bg)',      desc: 'Score ≥ 60 — urgent symptoms' },
                { label: 'Medium',   color: 'var(--medium)',    bg: 'var(--medium-bg)',    desc: 'Score ≥ 30 — moderate case' },
                { label: 'Low',      color: 'var(--low)',       bg: 'var(--low-bg)',       desc: 'Score < 30 — routine visit' },
              ].map(t => (
